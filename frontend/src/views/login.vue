...
import { siteLogin } from '@/api/site'
export default {
  name: 'login',
  data() {
    return {
  ...
  methods: {
    // 获取用户名密码
    getuserpwd() {
      // 仅当用户之前主动勾选“记住密码”时才回填；否则不自动回填，避免历史 cookie 一直占用
      if (this.rememberpwd && getCookie('user') && getCookie('pwd')) {
    ...
    submitForm(formName) {
      this.$refs[formName].validate(valid => {
        if (valid) {
          this.logining = true
          siteLogin({ username: this.ruleForm.username, password: this.ruleForm.password })
            .then(({ data }) => {
              if (data && data.success) {
                ...
                // 跳转按角色
                if (user.role === 'ADMIN') this.$router.push({ path: '/admin/users' })
                else if (user.role === 'MERCHANT') this.$router.push({ path: '/merchant/shop' })
